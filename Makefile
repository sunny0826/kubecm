...
GO_FLAGS=-ldflags="-X ${VERSION_PKG}.kubecmVersion=$(KUBECM_VERSION) -X ${VERSION_PKG}.gitRevision=$(GITCOMMIT) -X ${VERSION_PKG}.buildDate=$(shell date -u +'%Y-%m-%d')"
GO=go
...
ifeq ($(GOOS), linux)
	GO_FLAGS=-ldflags="-linkmode external -extldflags -static -X ${VERSION_PKG}.kubecmVersion=$(KUBECM_VERSION) -X ${VERSION_PKG}.gitRevision=$(GITCOMMIT) -X ${VERSION_PKG}.buildDate=$(shell date -u +'%Y-%m-%d')"
endif
